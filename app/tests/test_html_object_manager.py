...
from src.http_management.http_manager import HttpObjectManager
...
class TestHtmlObjectManager:
    ...
    def html_manager_object(self):
        if get_test_env_setting() == 'live':
            return HttpObjectManager(itjobswatch_home_page_url())
        else:
            return HttpObjectManager(itjobswatch_home_page_test_file())
